...
export default function ScheduleViewerWithEditor({
  schedule,
}: {
  schedule: Timetable
}) {
  ...
  const addCustomization = useMutation({
    ...
    mutationFn: (data: Partial<CustomizationData>) => {
      const promise = addCustomLesson({
        customizationData: data as CustomizationDataPayload,
      })
      toast.promise(promise, {
        loading: "Обновляем...",
        error: "Произошла ошибка",
  ...
  const editCustomization = useMutation({
    mutationKey: ["customLesson", "add"],
    mutationFn: (data: Partial<CustomizationData>) => {
      const promise = editCustomLesson({
        customizationData: data as CustomizationDataPayload & { id: number },
      })
      toast.promise(promise, {
        loading: "Обновляем...",
        error: "Произошла ошибка",
        success: "Пара изменена",
      })
      return promise
    },
    onSuccess: (data: unknown) => {
      ...
      const week = (data as any)?.weekNumber || 0
      if (week) queryClient.invalidateQueries({ queryKey: ["schedule", week] })
      else queryClient.invalidateQueries({ queryKey: ["schedule"] })
    },
  })
      ...
      {isDeleteDialogOpen && (
        <DialogContent showCloseButton={false}>
          <DialogHeader>
            ...
            <DialogDescription>
              Данное действие отменит все изменения, примянённые к этой паре
              <br />
          ...
          <DialogFooter>
            ...
            <DialogClose asChild>
              ...
              >
                {lesson?.original?.id ? "Сбросить" : "Удалить"}
              </Button>
            </DialogClose>
          </DialogFooter>
        </DialogContent>
