...
import {
  ScheduleLesson,
  ScheduleLessonWindow,
} from "@/client/components/ScheduleLesson"
...
export default function ScheduleViewer({
  ...
  let columnHeight = schedule.days.reduce((a, day) => {
    const t = day.lessons.reduce(
      (b, lesson) => (b > lesson.dayTimeSlot ? b : lesson.dayTimeSlot),
      0,
    )
    return a > t ? a : t
  }, 0)
  ...
  function dayHeader(day: (typeof schedule.days)[number]) {
    ...
    return (
      <div
        key={`${dayIndex}div${date}`}
        className="snap-end rounded-lg border-2 border-cyan-600 bg-cyan-900 p-2 font-bold text-white sm:min-w-0"
      >
    ...
    return (
      <>
        ...
        {new Array(columnHeight).fill(0).map((_, index) => {
          const lesson =
            day.lessons.find((i) => i.dayTimeSlot == index + 1) ?? null
          if (!lesson)
            return (
              <ScheduleLessonWindow
                key={`lesson_${dayIndex}.${index}`}
                time={{
                  weekNumber: day.week,
                  weekday: day.weekday,
                  dayTimeSlot: index + 1,
                }}
                hasMenu={editingEnabled}
              />
            )
          return (
            <ScheduleLesson
          ...
          return (
            <>
              <button
                ...
                key={`${day.week}.${day.weekday}navdiv`}
                className={
                  `flex-1 rounded-lg py-1 px-2 font-bold border-2 ${borderColor} bg-cyan-900 text-white` +
    ...
    if (columnHeight === 0) {
      ...
      return (
        <div
          ...
          className={
            "sticky left-0 flex flex-col justify-center rounded-lg p-2 font-bold min-w-16 sm:min-w-0 border-2 border-cyan-600 bg-cyan-900 text-white"
          }
        >
    ...
    if (columnHeight === 0) {
      ...
      if (isMobile) {
        ...
        return (
          <>
            <div
              key="dayLabelNoLessons"
              className="min-w-80 snap-end rounded-lg border-2 border-cyan-600 bg-cyan-900 p-2 font-bold text-white sm:min-w-0"
            >
            ...
            <div key="noLessonsCard" className="flex flex-col gap-1">
              <div className={"flex-1 py-20 " + s.cardStyle}>
                ...
                <div className="flex h-full flex-col justify-center px-1 text-left">
                  <p className="block text-center text-4xl font-bold">
                    Пар нет :D
                  </p>
                </div>
              </div>
    ...
    return (
      <>
        {schedule.days.map((day) => (
          <ScheduleDay key={day.beginTime.toString()} day={day} />
        ))}
      </>
    )
  }
  ...
  return (
    <div className="relative flex w-full flex-col items-stretch gap-2 p-2 text-base">
      ...
      */}
      ...
      <div
        className={
          "grid snap-x snap-proximity grid-flow-col  gap-1 overflow-x-auto scroll-smooth " +
          (!isMobile
      ...
      >
        <div
          key="timeLabel"
          className={
            "sticky left-0 snap-start flex flex-col justify-center rounded-lg p-2 font-bold border-2 border-cyan-600 bg-cyan-900 text-white"
          }
        >
          Время
        </div>
