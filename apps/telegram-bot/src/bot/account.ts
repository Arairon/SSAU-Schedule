...
async function start(ctx: Context, userId: number | bigint) {
  const user = await getUser(ctx, { required: false })
  if (!user)
  ...
  await ctx.reply(
    `\
Добро пожаловать!
Вы можете получить расписание по неделям, просто написав <b>номер группы</b> или <b>фамилию преподавателя</b> в чат.
По кнопке ниже вы можете пройти быструю настройку, войти в личный кабинет и закрепить за собой группу.
Вход в личный кабинет так же позволит получать информацию об ИОТах.

Краткий список команд:
/schedule - получить расписание на текущую неделю
/ics - получить ссылку на ICS календарь (очень рекомендую)
/options - настройки
/help - информация о боте
/bug [текст] - отправить сообщение об ошибке или предложении по улучшению бота

Если вы столкнулись с какой либо ошибкой или у вас есть предложение по улучшению бота - пожалуйста не стесняйтесь написать администратору бота или в чат новостного канала.

Бот вдохновлён @l9_stud_bot
Новостной канал: https://t.me/arai_ssau_bot_news
Исходный код: https://github.com/arairon/ssau-schedule
${env.SCHED_BOT_ADMIN_CONTACT !== "@arairon" ? `Администратор бота: ${env.SCHED_BOT_ADMIN_CONTACT}\n` : ""}\
Автор бота: @arairon
`,
    {
...
‼️<b>Файл по этой ссылке не для скачивания</b>
Содержимое файла будет обновляться вместе с расписанием и вашими настройками.
Эту ссылку можно добавить в любой календарь, поддерживающий ics.
На <b>айфоне</b> календарь поддерживает ics по умолчанию.
На <b>андройде</b> можно использовать <a href="icsx5.bitfire.at">ICSx5</a> (<a href="https://play.google.com/store/apps/details?id=at.bitfire.icsdroid">GPlay</a>) или добавить ссылку в гугл календарь из браузера
 `,
  ...
  commands.command("help", "Информация о боте", async (ctx) => {
    if (ctx.chat.type !== "private") return
    const user = await getUser(ctx)
    const isGroupSet = user && user.groupId

    return ctx.reply(
      `\
❓ Что делает этот бот?
Здесь можно получить расписание групп или преподавателей, а так же получать уведомления о парах и ссылки на онлайн занятия.

Просто получить интересующее расписание можно просто написав номер группы или фамилию преподавателя в чат.
Долго листать до искомой недели? Просто отправьте номер недели в чат и бот пролистает всё за вас.

${!isGroupSet
        ? `Почти все следующие пункты применимы только если вы настроили группу в /options, настоятельно рекомендую это сделать :]`
        : "Все настройки можно изменить через /options"}

Список команд:
/login - Вход в ЛК, позволяет получать информацию о ИОТах 
/logout - выход из ЛК
/options - Настройки бота (группа, уведомления, темы и т.п.)
/schedule - Получить расписание на текущую неделю
/ics - Получить ссылку на ICS календарь (очень рекомендую)
/help - Информация о боте
/bug [текст] - Отправить сообщение об ошибке или предложении по улучшению бота
/today - Получить расписание на сегодня. Очень удобно для получения ссылок на онлайн занятия
<b>"дд.мм"</b> - Получить расписание на конкретный день (например "01.09")
<b>"1"</b> - Получить расписание на конкретную неделю (1 - 52)
<b>"6101-090301D"</b> - Получить расписание группы по её номеру (можно частично, например "6101-09" или "610")
<b>"Иванов Иван"</b> - Получить расписание преподавателя по его ФИО (можно частично, например "Иванов" или "Ива")
