...
function App() {
  const authQuery = useAuth({ tg: true, token: window.localStorage.getItem("sched-token") || "" })
  const { isAuthorized, error: authError } = useAuthState()

  if (authQuery && authQuery.isLoading) {
    return (
...
function Schedule() {
  ...
  if (weekNumber < 52 && !error)
    queryClient.prefetchQuery({
      ...
      queryFn: () => getSchedule({ week: weekNumber + 1 }),
      staleTime: 300_000,
      retry: false,
    })
