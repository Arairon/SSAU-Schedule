...
import { parse as tgParse, validate as tgValidate } from "@tma.js/init-data-node";
import { env } from "../../env";
...
export async function registerAuth(fastify: FastifyInstance) {
  ...
  fastify.addHook("onRequest", async (req, res) => {
    ...
    if (refreshToken) {
      ...
      // req.setDecorator
      log.error("refreshToken was passed, but is not implemented on server yer")
      return
    } else {
      // Fall through to regular auth
      log.debug("User failed to provide valid jwt or refreshToken, trying to re-auth")
    }
    ...
    if ((!authData || authData === "null") && env.NODE_ENV === "development") {
      ...
      res.setCookie("accessToken", jwt.sign({
        userId: 1,
    ...
    switch (authType) {
      case 'tma': {
        try {
          ...
          const user = await db.user.findUnique({ where: { tgId: data.user.id } })
          if (!user) {
          ...
          res.setCookie("accessToken", jwt.sign(auth as object,
            env.SCHED_JWT_SECRET, { expiresIn: "1h" }
          ), { path: "/api/v0" })
          // Not using refreshTokens for tma auth
          req.setDecorator("authData", auth)
        } catch (e) {
          res.header("authorization-error", JSON.stringify(e))
          log.warn("Unable to authorize user: " + JSON.stringify(e), { user: "req-tma" })
        }
        break;
      }
      case 'Bearer': {
  ...
  fastify.post("/login", {}, async (req: FastifyRequest<{ Body: { login: string, password: string } }>, res) => {
