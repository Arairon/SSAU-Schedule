...
export async function routesCustomLesson(fastify: FastifyInstance) {
  ...
  fastify.post("/", {}, async (req: FastifyRequest<{ Body: unknown }>, res) => {
  ...
  fastify.delete(
    "/:lessonId",
    { schema: { params: lessonIdParamSchema } },
    async (req: FastifyRequest<{ Params: { lessonId: number } }>, res) => {
      ...
      if (
        ...
        return res
          .status(404)
          .send("CustomLesson with such id belonging to you not found");
      }
  ...
  fastify.put("/", {}, async (req: FastifyRequest<{ Body: unknown }>, res) => {
    const auth: AuthData = req.getDecorator("authData");
    if (!auth) return res.status(403).send("Unauthorized");
    const user = (await db.user.findUnique({ where: { id: auth.userId } }))!;
