...
import { invalidateDailyNotificationsForTarget, scheduleDailyNotificationsForUser } from "../../lib/tasks";


export async function routesNotifications(fastify: FastifyInstance) {

  fastify.post("/reschedule", {},
    async (req: FastifyRequest<{ Body: unknown }>, res) => {
      ...
      res.status(200).send({removed: invResult.count, added: updResult?.count ?? -1})
