...
export const CustomizationDataSchemaPartial = CustomizationDataSchema.partial().requiredFor("weekNumber", "weekday", "dayTimeSlot")
export type CustomizationDataPartial = s.infer<typeof CustomizationDataSchemaPartial>
...
function normalizeCustomLessonData(data: CustomizationDataPartial) {
  ...
  return Object.assign({}, data, {
    date: lessonDate,
    beginTime: new Date(lessonDate.getTime() + TimeSlotMap[data.dayTimeSlot].beginDelta),
    endTime: new Date(lessonDate.getTime() + TimeSlotMap[data.dayTimeSlot].endDelta),
  })
}

export async function addCustomLesson(user: User, customData: CustomizationDataPartial & { id?: null | undefined }) {
  log.debug(`Adding custom lesson (${customData.lessonInfoId ?? '0'}/${customData.lessonId}) with ${JSON.stringify(customData)}`, { user: user.id })

  const data = normalizeCustomLessonData(customData)
  data.userId = user.id;

  if (data.lessonInfoId) {
    const lessonsToOverride = await db.lesson.findMany({ where: { infoId: data.lessonInfoId } })
...
export async function deleteCustomLesson(user: User, lessonId: number) {
  ...
  const target = await db.customLesson.findUnique({ where: { id: lessonId, userId: user.id } })
  if (!target) return null;

  if (target.lessonInfoId) {
    return await db.customLesson.deleteMany({ where: { lessonInfoId: target.lessonInfoId } })
  }
  return await db.customLesson.delete({ where: { id: target.id } })
}

export async function editCustomLesson(user: User, customData: CustomizationDataPartial & { id: number }) {
  log.debug(`Editing ${customData.lessonInfoId ?? '0'}/${customData.lessonId} with ${JSON.stringify(customData)}`, { user: user.id })
